...
class ChecklistViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> ChecklistItemCell{
        ...
        configureText(for: cell as! ChecklistItemCell, withItem: (list.items[indexPath.row]))
        configureCheckmark(for: cell as! ChecklistItemCell, withItem: (list.items[indexPath.row]))
...
extension ChecklistViewController : ItemDetailViewControllerDelegate{
    ...
    func itemDetailViewController(_ controller: ItemDetailViewController, didFinishEditingItem item: ChecklistItem) {
        tableView.reloadRows(at: [IndexPath(row: list.items.index(where: { $0 === item })!, section: 0)], with: .none)
    ...
    func itemDetailViewController(_ controller: ItemDetailViewController, didFinishAddingItem item: ChecklistItem) {
        list.items.append(ChecklistItem(text: item.text))
        tableView.insertRows(at: [IndexPath(row: (self.list.items.count) - 1 , section: 0)]  , with: .none)
        //saveChecklistItems()
        self.dismiss(animated: true, completion: nil)
    }
